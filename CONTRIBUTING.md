...
2. **Implement the API.** Match the existing surface: `encode`, `decode`, `try_decode`, the `*_many` batch variants, a bound-scope handle, and the options (`min_length`, `base_alphabet`, `previous_secrets`).
...
4. **Match the conventions.** Name tests after the behavior they exercise (no `test_coverage`-style names). Name the instance `scope_mask` / `scopeMask`, not `m`.

5. **Wire it up.** Add a directory README, a docs page, and a CI workflow modelled on the existing ones.
